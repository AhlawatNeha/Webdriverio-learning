describe('Ecommerce Application', async()=>{
    it('End to End test',async()=>{
       ...
       await browser.pause(5000)
        ...
        const sumOfProducts=(await Promise.all(await productPrices.map(async (productPrices) => parseInt(await productPrices.getText()).split(".")[1].trim()))).reduce((acc,price)=>acc+price,0)
